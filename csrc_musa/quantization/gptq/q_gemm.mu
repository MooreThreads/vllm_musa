...
)
{
    if (q_perm)
    {
        ...
        musaMalloc(&new_qweight, height / 32 * bit * width * sizeof(uint32_t));
        ...
        // Replace qweights
        musaMemcpyAsync(q_weight, new_qweight, height / 32 * bit * width * sizeof(uint32_t), musaMemcpyDeviceToDevice);
        // Cleanup
        musaDeviceSynchronize();
        musaFree(new_qweight);
    }
    dim3 blockDim, gridDim;
    blockDim.x = THREADS_X;
    blockDim.y = 1;
...
)
{
    ...
    (
        at::musa::getCurrentMUSABlasHandle(),
...
{
    const at::musa::OptionalMUSAGuard device_guard(device_of(q_weight));
